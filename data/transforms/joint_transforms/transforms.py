...
# TODO add color jittering
# TODO izvalidirati SAM
# TODO singlescale
class JitterRandomCrop(torch.nn.Module):
    ...
    @staticmethod
    def get_crop_params(
        img: Tensor, output_size: Tuple[int, int], centroid
        ...
        if not centroid:
            ...
            max_w = w - tw
            max_h = h - th
            i = torch.randint(c_i - th, c_i, size=(1,)).item()
            i = min(max_h, max(0, i))
            j = random.randint(c_j - tw, c_j)
            j = min(max_w, max(0, j))
    ...
    def forward(self, data):
        ...
        if centroid:
            ci, cj = centroid
            centroid = (int(ci * coef_), int(cj * coef_))
        ...
        i, j, h, w = self.get_crop_params(img, self.size, centroid)
...
class JointResize(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        img, lbl = data
        return (
            F.resize(img, self.size, Image.BILINEAR),
...
class AppendTransitionMatrix(torch.nn.Module):
    def __init__(self, T):
        super().__init__()
        self.T = T.cpu()
    ...
    def forward(self, data):
        ...
        # y_oh = torch.nn.functional.one_hot(y, self.num_classes + 1)[..., :self.num_classes].permute(3, 0, 1, 2)
        # C, _, H, W = y_oh.shape
        # y_oh = rearrange(y_oh, 'c b h w -> c (b h w)')
        # y_noisy = torch.mm(self.Tinv.to(y_oh), y_oh)
        # y_noisy = rearrange(y_noisy, 'c (b h w) -> b c h w', b=1, h=H, w=W)
        return x, y, b, self.T
