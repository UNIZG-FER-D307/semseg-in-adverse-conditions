...
class LabelToBoundaryClone(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        # if not (isinstance(img, torch.Tensor) and isinstance(lbl, torch.Tensor) and isinstance(label_distance_alphas, np.ndarray)):
        #     torch.save(img, 'img_fail.pth')
        #     torch.save(img, 'lbl_fail.pth')
        #     torch.save(img, 'alph_fail.pth')
        #     raise Exception('E moj mateju')
