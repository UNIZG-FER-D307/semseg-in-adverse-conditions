...
foggy_driving_mean = np.array([0.52644604, 0.52056108, 0.52190866])
foggy_driving_std = np.array([0.22403087, 0.23167806, 0.24031129])
...
class FoggyDriving(data.Dataset):
    ...
    def __init__(self, root, image_transform=None, target_transform=None, joint_transform=None):
        ...
        self.annotations_base = os.path.join(self.root, 'gtFine', 'test')

        self.files = recursive_glob(rootdir=self.annotations_base, suffix='_gtFine_labelTrainIds.png')
    ...
    def __getitem__(self, index):
        ...
        img_path = os.path.join(self.images_base, name.replace('_gtFine_labelTrainIds.png', '_leftImg8bit.png'))
    ...
    def create_img_lbl_list(self):
        return [
            (os.path.join(self.images_base, '/'.join(lbl_path.split('/')[-2:]).replace('_gtFine_labelTrainIds.png', '_leftImg8bit.png')), lbl_path)
