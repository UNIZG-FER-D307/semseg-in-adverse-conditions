...
def class_centroids_image(item, tile_size, num_classes, id2trainid):
    ...
    mask_copy = mask.copy()
    if type(id2trainid) is np.ndarray or type(id2trainid) is torch.Tensor:
        ids = id2trainid.shape[0]
        # for k, v in id2trainid.items():
        for i in range(ids):
            mask[mask_copy == i] = id2trainid[i]
    ...
    for x_offs, y_offs in tile_locations:
        ...
        for class_id in range(num_classes):
            if class_id in patch:
                ...
                centroid_y, centroid_x = ndimage.measurements.center_of_mass(
                    patch_class
                )
...
def unpooled_class_centroids_all(items, num_classes, id2trainid, tile_size=1024):
    """
    Calculate class centroids for all classes for all images for all tiles.
    items: list of (image_fn, label_fn)
    tile size: size of tile
    ...
    for image, label in items:
        new_centroids = class_centroids_image(
            (image, label), tile_size, num_classes, id2trainid
        )
...
def class_centroids_all(items, num_classes, id2trainid, tile_size=1024):
    ...
    pooled_centroids = pooled_class_centroids_all(
        items, num_classes, id2trainid, tile_size
    )
...
# [4, 9, 12, 17, 18]
def build_epoch(
    imgs, centroids, num_classes, class_uniform_pct, oversampled_classes=[]
):
    """
    ...
    num_per_class = int(
        (num_epoch * class_uniform_pct) / (num_classes + len(oversampled_classes))
    )
    num_rand = num_epoch - num_per_class * (num_classes + len(oversampled_classes))
    # create random crops
    imgs_uniform = random_sampling(imgs, num_rand)

    # now add uniform sampling
    for class_id in range(num_classes):
        string_format = "cls %d len %d" % (
            class_id,
            len(centroids[class_id]) if class_id in centroids.keys() else 0,
        )
        logging.info(string_format)

    per_class_samples = [num_per_class] * num_classes
    for cid in oversampled_classes:
        per_class_samples[cid] = per_class_samples[cid] * 2

    missing_images = 0
    for class_id in range(num_classes):
        centroid_len = len(centroids[class_id]) if class_id in centroids.keys() else 0
        if centroid_len == 0:
            missing_images += per_class_samples[class_id]
            pass
        else:
            class_centroids = random_sampling(
                centroids[class_id], per_class_samples[class_id]
            )
            imgs_uniform.extend(class_centroids)

    # compensate for missing classes
    imgs_uniform.extend(random_sampling(imgs, missing_images))
    assert len(imgs) == len(imgs_uniform)
    return imgs_uniform
