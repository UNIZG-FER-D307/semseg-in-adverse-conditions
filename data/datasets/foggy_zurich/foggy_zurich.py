...
foggy_zurich_mean = np.array([0.55309306, 0.54356853, 0.5543626])
foggy_zurich_std = np.array([0.22886978, 0.23141141, 0.24247366])
...
class FoggyZurich(data.Dataset):
    ...
    def __init__(self, root, image_transform=None, target_transform=None, joint_transform=None):
        ...
        self.annotations_base = os.path.join(self.root, 'gt_labelTrainIds')
    ...
    def create_img_lbl_list(self):
        return [
            (os.path.join(self.images_base, '/'.join(lbl_path.split('/')[-2:])), lbl_path)
