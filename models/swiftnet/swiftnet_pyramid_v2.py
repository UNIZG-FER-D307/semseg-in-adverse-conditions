...
class _BNReluConv(nn.Sequential):
    def __init__(self, num_maps_in, num_maps_out, k=3, batch_norm=True, bias=True, dilation=1):
...
class ResNet(nn.Module):
    ...
    def _dense_forward(self, x: Tensor) -> list:
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
...
class JitterPyramid(nn.Module):
    def __init__(self, network, upsample_dims):
        super(JitterPyramid, self).__init__()
        self.network = network
        self.bn_modules_s0 = self._extract_bn()
        self.bn_modules_s1 = copy.deepcopy(self.bn_modules_s0)
        self.bn_modules_s2 = copy.deepcopy(self.bn_modules_s1)
        self.bn_modules_s3 = copy.deepcopy(self.bn_modules_s1)

        chnls = self._out_chnls()
        self.up0 = _create_upsamping_bn_relu_conv(chnls, upsample_dims[:4])
        self.up1 = _create_upsamping_bn_relu_conv(chnls, upsample_dims[1:5])
        self.up2 = _create_upsamping_bn_relu_conv(chnls, upsample_dims[2:6])
        self.up3 = _create_upsamping_bn_relu_conv(chnls, upsample_dims[3:])


    def _extract_bn(self):
        bn_modules = nn.ModuleDict()
        for k, v in self.network.named_modules():
            if 'bn' in k:
    ...
    def forward(self, x):
        self._switch_bn(self.bn_modules_s0)
        x_ = F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=False)
        outs_s0 = self.network(x_)

        self._switch_bn(self.bn_modules_s1)
        outs_s1 = self.network(x)

        self._switch_bn(self.bn_modules_s2)
        x = F.interpolate(x, scale_factor=0.5, mode='bilinear', align_corners=False)
        outs_s2 = self.network(x)

        self._switch_bn(self.bn_modules_s3)
        x = F.interpolate(x, scale_factor=0.5, mode='bilinear', align_corners=False)
        outs_s3 = self.network(x)

        p0_up = [t(o) for o, t in zip(outs_s0, self.up0)]
        ...
        outputs = [
            p0_up[0],
            p0_up[1] + p1_up[0],
            p0_up[2] + p1_up[1] + p2_up[0],
            p0_up[3] + p1_up[2] + p2_up[1] + p3_up[0],
...
class Upsample(nn.Module):
    def __init__(self, num_features, up_sizes):
        super(Upsample, self).__init__()
        upsample_layers = []
        for i in range(len(up_sizes)):
            upsample = _Upsample(num_features, up_sizes[i])
...
class JitterPyramidSemSeg(nn.Module):
    def __init__(self, backbone, num_classes=19, upsample_dims=128):
        super(JitterPyramidSemSeg, self).__init__()
        self.num_classes = num_classes
        up_dims = [upsample_dims] * 7
        self.backbone = JitterPyramid(backbone, upsample_dims=up_dims)
...
if __name__ == '__main__':
    ...
    m2 = JitterPyramidSemSeg(m, 256)
    print(m2(torch.randn(1, 3, 256, 256)).shape)
    print(parameter_count(m2))

# __getattr__('35').__getattr__('bn3')
