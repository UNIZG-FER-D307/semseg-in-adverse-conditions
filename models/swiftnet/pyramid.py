...
class Pyramid(nn.Module):
    def __init__(self, network, upsample_dims):
        ...
        self.bn_modules_s1 = self._extract_bn()
...
class PyramidOpenClip(nn.Module):
    def __init__(self, network, upsample_dims):
        super(PyramidOpenClip, self).__init__()
        self.network = network
        self.bn_modules_s1 = self._extract_bn()
        self.bn_modules_s2 = copy.deepcopy(self.bn_modules_s1)
        self.bn_modules_s3 = copy.deepcopy(self.bn_modules_s1)
    ...
    def forward(self, x):
        self._switch_bn(self.bn_modules_s1)
        outs_s1, lf1 = self.network(x)
        outs_s1[-1] = lf1
        self._switch_bn(self.bn_modules_s2)
        x = F.interpolate(x, scale_factor=0.5, mode="bilinear", align_corners=False)
        outs_s2, lf2 = self.network(x)
        outs_s2[-1] = lf2
        self._switch_bn(self.bn_modules_s3)
        x = F.interpolate(x, scale_factor=0.5, mode="bilinear", align_corners=False)
        outs_s3, lf3 = self.network(x)
        outs_s3[-1] = lf3
        p1_up = [t(o) for o, t in zip(outs_s1, self.up1)]
        p2_up = [t(o) for o, t in zip(outs_s2, self.up2)]
        p3_up = [t(o) for o, t in zip(outs_s3, self.up3)]
...
class PyramidSemSegOpenClip(nn.Module):
    def __init__(
        self,
        backbone,
        num_classes=19,
        ...
        super(PyramidSemSegOpenClip, self).__init__()
        self.num_classes = num_classes
        self.use_aux = use_aux
        up_dims = [upsample_dims] * 6
        self.backbone = PyramidOpenClip(backbone, upsample_dims=up_dims)
        self.upsample = Upsample(upsample_dims, up_dims, num_classes, use_aux)
    ...
    def prepare_optim_params(self):
        ...
        if self.embeddings is None:
            return backbone_params, upsample_params + list(self.logits.parameters())
        else:
            return backbone_params, upsample_params
...
class PyramidSemSegSelfSup(nn.Module):
    ...
    def prepare_optim_params(self):
        ...
        )  # + list(self.selfsup_predictor.parameters())
...
class NoisyPyramidSemSeg(nn.Module):
    ...
    def prepare_optim_params(self):
        return list(self.backbone.parameters()), list(
            self.upsample.parameters()
        ) + list(self.logits.parameters()) + list(self.t_matrix.parameters())
...
class PyramidSemSegTH(nn.Module):
    ...
    def forward(self, x):
        ...
        ood_out = F.interpolate(
            ood_logits, (H, W), mode="bilinear", align_corners=False
        )
