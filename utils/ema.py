...
class EMA(pl.Callback):
    ...
    def get_state_dict(pl_module: pl.LightningModule):
        ...
        """
        bn_stat_keys = ['running_var', 'running_mean', 'num_batches_tracked']
        ema_state = OrderedDict(
            filter(lambda i: i[0].split('.')[-1] not in bn_stat_keys, pl_module.model.state_dict().items()))
        return ema_state
    ...
    def on_train_batch_end(self, trainer: "pl.Trainer", pl_module: pl.LightningModule, *args, **kwargs) -> None:
        ...
        with torch.no_grad():
            for key, value in self.get_state_dict(pl_module).items():
                ...
                ema_value.copy_(self.decay * ema_value + (1. - self.decay) * value.to(ema_value), non_blocking=True)
    ...
    def on_validation_end(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule") -> None:
        ...
        pl_module.model.load_state_dict(self.original_state_dict, strict=False)
